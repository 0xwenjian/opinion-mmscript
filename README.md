...
*   **动态最优位置**：每 3 秒扫描订单簿，自动计算满足保护金额（`min_protection_amount`）的最靠前安全位置，双向调整（前进/后退）。
*   **买1避让机制**：永远不会直接挂在买1价格上，始终保持至少一档缓冲。
...
Copyright © 2026 0xwenjian.
本项目采用 [Non-Commercial License](LICENSE)（非商业用途许可证）。
严禁任何形式的商业使用、转售或利用本项目牟利。
